...
날짜 바인딩은 Avalonia CalendarDatePicker 계약과 같은 DateTime?를 사용하고, 저장 직전에 날짜와 시간을 UTC DateTimeOffset으로 조합합니다. 입력 영역은 세로 scrollbar를 제공하며 24시간제 시간 Picker와 UTC 표시를 고정 폭으로 배치합니다. 하단 명령은 좁은 창에서 줄바꿈되고 상태 문구는 별도 행에 표시됩니다.
